        ...
        <div class="admin_form">
            <a-form-model ref="form" :model="form" :rules="rules" :label-col="{ span: 5 }" :wrapper-col="{ span: 12 }">
                ...
                <a-form-model-item label="名称" prop="name" v-show="showFormItem['name']">
                    <a-input v-model="form.name" :size="sysSize"></a-input>
                </a-form-model-item>
                <a-form-model-item label="连接类型" prop="conn_type" v-show="showFormItem['conn_type']">
                    <a-input v-model="form.conn_type" :size="sysSize"></a-input>
                </a-form-model-item>
                <a-form-model-item label="TNS" prop="tns" v-show="showFormItem['tns']">
                    <a-textarea v-model="form.tns" :auto-size="{ minRows: 3, maxRows: 5 } :size="sysSize"" />
                </a-form-model-item>
                <a-form-model-item label="驱动" prop="driver" v-show="showFormItem['driver']">
                    <a-input v-model="form.driver" :size="sysSize"></a-input>
                </a-form-model-item>
                <a-form-model-item label="主机名" prop="host" v-show="showFormItem['host']">
                    <a-input v-model="form.host" :size="sysSize"></a-input>
                </a-form-model-item>
                <a-form-model-item label="端口号" prop="port" v-show="showFormItem['port']">
                    <a-input-number v-model="form.port" :min="1024" @change="onChangePort" :size="sysSize" />
                </a-form-model-item>
                <a-form-model-item label="数据库名" prop="database" v-show="showFormItem['database']">
                    <a-input v-model="form.database" placeholder="请输入数据库名" :size="sysSize"></a-input>
                </a-form-model-item>
                <a-form-model-item label="用户名" prop="username" v-show="showFormItem['username']">
                    <a-input v-model="form.username" placeholder="请输入用户名" :size="sysSize"></a-input>
                </a-form-model-item>
                ...
                <a-form-model-item label="描述" prop="description" v-show="showFormItem['description']">
                    <a-textarea v-model="form.description" :auto-size="{ minRows: 3, maxRows: 5 }" :size="sysSize" />
                </a-form-model-item>
                <a-form-model-item :wrapper-col="{ span: 12, offset: 5 }">
                    <a-row :gutter="8">
                        <a-col :span="6">
                            <a-button type="primary" :size="sysSize" @click="handleSubmit(false)">提交</a-button>
                        </a-col>
                        <a-col :span="18">
                            <a-button v-if="!test" type="dashed" :size="sysSize" @click="handleSubmit(true)" :loading="loading_test">测试连接</a-button>
                            <a-button v-if="test" type="dashed" :size="sysSize" @click="handleSubmit(true)" icon="check">连接成功</a-button>
                        </a-col>
                    </a-row>
                </a-form-model-item>
            </a-form-model>
...
export default {
    ...
    methods: {
            ...
            this.$refs.form.validate(valid => {
                if (valid) {
                    if(test){
                        ...
                        this.$post(this.$api.adminTestConnection,this.form).then(res=>{
                        ...
                        let api = this.$apiHandle(this.$api.adminConnections,this.id);
                        if(api.status){
        ...
        get_form(){
            this.$get(this.$api.adminConnections+'/'+this.id).then(res=>{
        ...
        getDatasourceSelector(params){
            ...
            this.$get(this.$api.adminDatasources, params).then(res=>{
