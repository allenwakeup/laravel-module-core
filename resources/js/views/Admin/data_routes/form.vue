        ...
        <div class="admin_form">
            <a-form-model ref="form" :model="form" :rules="rules" :label-col="{ span: 5 }" :wrapper-col="{ span: 12 }">
                <a-form-model-item label="名称" prop="name">
                    <a-input v-model="form.name" placeholder="请输入路径名称" :size="sysSize">
                ...
                <a-form-model-item label="简称" prop="short">
                    <a-input v-model="form.short" placeholder="请输入路径简称" :size="sysSize">
                ...
                <a-form-model-item label="别名" prop="alias">
                    <a-input v-model="form.alias" placeholder="请输入路径别名" :size="sysSize">
                ...
                <a-form-model-item label="首表名称" prop="from">
                    <a-input v-model="form.from" placeholder="请输入首表名称" :size="sysSize">
                ...
                <a-form-model-item label="尾表名称" prop="to">
                    <a-input v-model="form.to" placeholder="请输入尾表名称" :size="sysSize">
                ...
                <a-form-model-item label="目标表名" prop="output">
                    <a-input v-model="form.output" placeholder="请输入数据库完整表名。注：表名一旦设置，均不可修改，请谨慎处理。" :size="sysSize">
                ...
                <a-form-model-item label="描述" prop="description">
                    <a-textarea v-model="form.description" placeholder="请输入描述信息。" :auto-size="{ minRows: 3, maxRows: 5 }" :size="sysSize" />
                </a-form-model-item>
                <a-form-model-item :wrapper-col="{ span: 12, offset: 5 }">
                    <a-button type="primary" @click="handleSubmit" :size="sysSize">提交</a-button>
                </a-form-model-item>
            </a-form-model>
    ...
    methods: {
        handleSubmit(){
            ...
            this.$refs.form.validate(valid => {
                if (valid) {
                    let api = this.$apiHandle(this.$api.adminDataRoutes,this.id);
                    if(api.status){
        ...
        getConnectionSelector(params){
            this.$get(this.$api.adminConnections, params).then(res=>{
