	        ...
	        <a-table
		        ...
		        <template slot="title" slot-scope="currentPageData">
                    ...
                    <div class="admin_table_handle_btn">
                        <a-button @click="$router.push($moduleUrl('datasources/form'))" type="primary" :size="sysSize" icon="plus">添加</a-button>
                        <a-button class="admin_delete_btn" type="danger" :size="sysSize" icon="delete" @click="handleRemoveTableRows">批量删除</a-button>
                    </div>
                </template>

                <span slot="action" slot-scope="rows">
                    <a-button type="link" icon="edit" :size="sysSize" @click="$router.push($moduleUrl('datasources/form/'+rows.id))">编辑</a-button>
                </span>
            </a-table>
...
export default {
    ...
    data() {
      return {
          table: {
              actions: {
                  list: this.$api.adminDatasources,
                  remove: this.$api.adminDatasources
              },
              columns: [
                  ...
                  {title:'操作',fixed:'right',scopedSlots: { customRender: 'action' }, width: 90},
              ],
          },
          search: {
              fields: [
