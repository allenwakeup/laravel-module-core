                ...
                <a-row>
                    <a-col :span="12">
                        <a-form-model-item label="左表名称" :labelCol="{span:4}" :wrapperCol="{span:16}" prop="left">
                            <a-input v-model="form.left" placeholder="请输入左表名称" :size="sysSize" />
                        </a-form-model-item>
                    </a-col>
                    <a-col :span="12">
                ...
                <a-row>
                    <a-col>
                        <a-form-model-item label="左表模板" :labelCol="{span:2}" :wrapperCol="{span:16}" prop="left_tpl">
                            <a-textarea v-model="form.left_tpl" :size="sysSize" placeholder="用来显示的列，支持字段名+转换规则，
                ...
                <a-row>
                    <a-col :span="12">
                        <a-form-model-item label="右表名称" :labelCol="{span:4}" :wrapperCol="{span:16}" prop="right">
                            <a-input v-model="form.right" placeholder="请输入右表名称" :size="sysSize" />
                        </a-form-model-item>
                    </a-col>
                    <a-col :span="12">
                ...
                <a-row>
                    <a-col>
                        <a-form-model-item label="右表模板" :labelCol="{span:2}" :wrapperCol="{span:16}" prop="right_tpl">
                            <a-textarea v-model="form.right_tpl" :size="sysSize" placeholder="用来显示的列，支持字段名+转换规则，
如：表列名有name、department，想要显示成「department」name前两个字符的拼接格式，
可以设置成 department::prepend:「|append:」+name::substr:0,2
" :auto-size="{ minRows: 5, maxRows: 8 }" />
                        </a-form-model-item>
                ...
                <a-row>
                    <a-col :span="12">
                        <a-form-model-item label="多态前缀" :labelCol="{span:4}" :wrapperCol="{span:16}" prop="name">
                            <a-input v-model="form.name" :size="sysSize" placeholder="请输入多态前缀">
                    ...
                    <a-col :span="12">
                        <a-form-model-item label="表名" :labelCol="{span:4}" :wrapperCol="{span:16}" prop="table">
                            <a-input v-model="form.table" :size="sysSize" disabled/>
                        </a-form-model-item>
                    </a-col>
                </a-row>
                <a-row>
                    <a-col :span="12">
                        <a-form-model-item label="Foreign Pivot Key" :labelCol="{span:4}" :wrapperCol="{span:16}" prop="foreign_pivot_key">
                            <a-input v-model="form.foreign_pivot_key" :size="sysSize" />
                        </a-form-model-item>
                    </a-col>
                    <a-col :span="12">
                        <a-form-model-item label="Related Pivot Key" :labelCol="{span:4}" :wrapperCol="{span:16}" prop="related_pivot_key">
                            <a-input v-model="form.related_pivot_key" :size="sysSize" />
                        </a-form-model-item>
                    </a-col>
                </a-row>
                <a-row>
                    <a-col :span="12">
                        <a-form-model-item label="Parent Key" :labelCol="{span:4}" :wrapperCol="{span:16}" prop="parent_key">
                            <a-input v-model="form.parent_key" placeholder="关联主表的主键名，如：id" :size="sysSize"/>
                        </a-form-model-item>
                    </a-col>
                    <a-col :span="12">
                        <a-form-model-item label="Related Key" :labelCol="{span:4}" :wrapperCol="{span:16}" prop="related_key">
                            <a-input v-model="form.related_key"  placeholder="关联副表的主键名，如：id" :size="sysSize"/>
                        </a-form-model-item>
                    </a-col>
                </a-row>
                ...
                <a-row>
                    <a-col>
                        <a-form-model-item label="描述" :labelCol="{span:2}" :wrapperCol="{span:16}" prop="description">
                            <a-textarea v-model="form.description" placeholder="这个数据映射是用来干啥的？什么都没写" :auto-size="{ minRows: 3, maxRows: 5 }" :size="sysSize" />
                        </a-form-model-item>
                    </a-col>
                </a-row>
                <a-row>
    ...
    methods: {
        handleSubmit(){
            ...
            this.$refs.form.validate(valid => {
                if (valid) {
                    let api = this.$apiHandle(this.$api.adminDataMaps,this.id);
                    if(api.status){
        ...
        get_form(){
            this.$get(this.$api.adminDataMaps+'/'+this.id).then(res=>{
                this.form = res.data;
            });
        },
        getDataRouteSelector(params){
            this.$get(this.$api.adminDataRoutes, params).then(res=>{
        ...
        getLeftTableSelector(params){
            this.$get(this.$api.adminDatabases, params).then(res=>{
                if(res.data && res.data.data){
